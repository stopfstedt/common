...
export default Component.extend(ValidationErrorDisplay, Validations, {
  ...
  associatedSchools: computed('cohorts.[]', async function(){
    ...
    if (isEmpty(cohorts)) {
      ...
      const cohortSchools = map(cohorts.toArray(), cohort => {
  ...
  availableInstructorGroups: computed('associatedSchools.[]', async function(){
    ...
    const allInstructorGropus = await map(associatedSchools, school => {
      return school.get('instructorGroups');
    });
    return allInstructorGropus.reduce((flattened, obj) => {
